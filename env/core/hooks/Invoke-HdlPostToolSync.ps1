...
try {
    ...
    Write-HdlHookDecision -Decision approve -Reason "No active HDL project inferred; multi-project state index synced" -Additional ([pscustomobject]@{
        state = "local/runtime/omx/state/hdl-workflow-state.json"
...
Write-HdlHookDecision -Decision approve -Reason "HDL workflow multi-project state index synced" -Additional ([pscustomobject]@{
