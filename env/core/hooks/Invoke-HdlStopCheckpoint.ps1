...
if ($project) {
    $currentState = Join-Path $project "work\memory\00_global\CURRENT_STATE.md"
    ...
    $memories = @(
        ...
        [ordered]@{
            ...
            summary = "Configuration lives under prj/<name>/work/config. Editable deliverables live under each project's output/. Legacy workspace data is migration input only."
        }
    )
}
else {
    ...
    foreach ($record in @($state.projects)) {
        $statePath = Join-Path $workspace ("prj\{0}\work\memory\00_global\CURRENT_STATE.md" -f $record.name)
...
$payload = [ordered]@{
    ...
    authority = "Project files are authoritative; local/runtime/omx is a multi-project runtime summary only."
    memories = $memories
}

Write-HdlJsonAtomic -Data $payload -Path (Join-Path $omx "project-memory.json")
Write-HdlHookDecision -Decision approve -Reason "HDL workflow multi-project checkpoint written" -Additional ([pscustomobject]@{
    active_project = if ($projectName) { $projectName } else { $null }
