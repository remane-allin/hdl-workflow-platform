...
function Find-HdlWorkspaceRoot {
    ...
    while ($current) {
        if ((Test-Path (Join-Path $current "env\rule\global\workspace_config.yaml")) -and
            (Test-Path (Join-Path $current "prj"))) {
            return $current
        }
    ...
    throw "Unable to locate HDL workflow workspace root from '$StartPath'."
}
...
function Resolve-HdlProjectRoot {
    ...
    if ($ProjectPath) {
        ...
        if (-not (Test-Path (Join-Path $resolved "output"))) {
            throw "ProjectPath is not an HDL workflow project: $resolved"
        }
        return $resolved
    }
    ...
    while ($current) {
        ...
        if ((Test-Path $projectMarker) -and (Test-Path (Join-Path $current "output"))) {
            $candidateRoot = Join-Path $workspace "prj"
    ...
    $projectsRoot = Join-Path $workspace "prj"
    $projects = @(Get-ChildItem -Path $projectsRoot -Directory | Where-Object {
        $config = Join-Path $_.FullName "work\config\project_config.yaml"
        (Test-Path $config) -and (Test-Path (Join-Path $_.FullName "output"))
    })
...
function Get-HdlProjectRecords {
    ...
    return @(Get-ChildItem -Path $projectsRoot -Directory | Where-Object {
        ...
        $projectName = $_.Name
        $statePath = Join-Path $_.FullName "work\memory\00_global\CURRENT_STATE.md"
        $loopStatePath = Join-Path $_.FullName "work\gates\loop_state.json"
        $activeNode = ""
        $latestSummary = ""
        ...
        [ordered]@{
            name = $projectName
            path = $_.FullName
            config = "prj/$projectName/work/config/project_config.yaml"
            output = "prj/$projectName/output"
            memory = "prj/$projectName/work/memory"
...
function Get-HdlOrphanProjectConfigs {
    param([Parameter(Mandatory = $true)][string]$WorkspaceRoot)
    ...
    return @(Get-ChildItem -Path $projectsRoot -Directory | Where-Object {
        (Test-Path (Join-Path $_.FullName "work\config\project_config.yaml")) -and
        (-not (Test-Path (Join-Path $_.FullName "project_scaffold.yaml")))
    } | Sort-Object Name | ForEach-Object {
        "prj/$($_.Name)/work/config/project_config.yaml"
    })
}
...
function Convert-HdlLoopNameToNode {
    ...
    switch ($LoopName) {
        "spec" { return "input" }
        "docparse" { return "work/docparse" }
        "loop1" { return "work/loop1_rtl_tb" }
        "loop2" { return "work/loop2_uvm" }
        "loop3" { return "work/loop3_fpga_proto" }
        "final" { return "output" }
...
function Write-HdlWorkspaceStateIndex {
    ...
    $workspace = (Resolve-Path $WorkspaceRoot).Path
    $stateDir = Join-Path $workspace "local\runtime\omx\state"
    ...
    if ($ActiveProject -and ($projectNames -contains $ActiveProject)) {
        $effectiveActiveProject = $ActiveProject
    }
    $state = [ordered]@{
        schema_version = 2
        workspace = $workspace
        updated_at = Get-HdlHookTimestamp
        authority = "Project-local memory under prj/<name>/work/memory is authoritative; local/runtime/omx is a multi-project runtime index only."
        active_project = $effectiveActiveProject
        projects = @($projects)
        orphan_project_configs = @(Get-HdlOrphanProjectConfigs -WorkspaceRoot $workspace)
    }
