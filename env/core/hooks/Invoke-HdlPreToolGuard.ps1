...
function Test-HdlPostChangeFrontdoorGenerated {
    param(
        ...
        [string[]]$RequiredSections = @("requirements", "fpga")
    )
    ...
    $designReport = Join-Path $ProjectRoot "output\reports\design\design_rule_and_architecture.md"
    $designManifest = Join-Path $ProjectRoot "output\reports\design\design_doc_manifest.json"
    if ((-not (Test-Path -LiteralPath $designReport)) -or (-not (Test-Path -LiteralPath $designManifest))) {
        return "$ScopeName require regenerating the requirements/design document with python -m hdlflow.cli generate-design-doc before formal source edits"
    }
    $designReportItem = Get-Item -LiteralPath $designReport
    $designManifestItem = Get-Item -LiteralPath $designManifest
    if (($designReportItem.LastWriteTimeUtc -lt $requestTime) -or ($designManifestItem.LastWriteTimeUtc -lt $requestTime)) {
        return "$ScopeName require generate-design-doc to be rerun after the approved change before formal source edits"
    }
    try {
        $manifest = Get-Content -LiteralPath $designManifest -Raw -ErrorAction Stop | ConvertFrom-Json
        ...
        if ($missingSections.Count -gt 0) {
            return "$ScopeName require generated document section(s): $($missingSections -join ', ')"
        }
    }
    catch {
        return "$ScopeName require a valid design_doc_manifest.json before formal source edits"
    }
    return ""
}
...
$designReportPattern = "output/reports/design"
if (($normalizedParserCommand -match $docparseRecordWritePattern) -and
    ($normalizedParserCommand -match $designReportPattern) -and
    ($normalizedParserCommand -notmatch "\bgenerate-design-doc\b")) {
    Write-HdlHookDecision -Decision block -Continue $false -Reason "design reports must be generated by python -m hdlflow.cli generate-design-doc after requirements-frontdoor-check passes"
    exit 0
}
...
try {
    ...
    $frontdoorGeneratorPattern = "generate-design-doc"
    ...
    if (($normalizedCommand -match $writePattern) -and
        ...
        Write-HdlHookDecision -Decision block -Continue $false -Reason "front-door source changes after a gate baseline require change control first; run python -m hdlflow.cli change-open, then record impact/approval before rerunning requirements-frontdoor-check, generate-design-doc, and the DocParse gate"
        exit 0
    }
    if (($normalizedCommand -match $writePattern) -and
        ...
        $postChangeReason = Test-HdlPostChangeFrontdoorGenerated -ProjectRoot $projectRoot -ApprovedRequest $approvedRequest -ScopeName "prototype verification changes" -RequiredSections @("requirements", "fpga")
    ...
    if (($normalizedCommand -match $sourceEditPattern) -and
        ...
        $postChangeReason = Test-HdlPostChangeFrontdoorGenerated -ProjectRoot $projectRoot -ApprovedRequest $approvedRequest -ScopeName "Loop1 RTL/TB requirement changes" -RequiredSections @("requirements", "rtl", "test_plan")
        if (-not [string]::IsNullOrWhiteSpace($postChangeReason)) {
            Write-HdlHookDecision -Decision block -Continue $false -Reason $postChangeReason
            exit 0
        }
    ...
    if (($normalizedCommand -match $sourceEditPattern) -and
        ...
        $postChangeReason = Test-HdlPostChangeFrontdoorGenerated -ProjectRoot $projectRoot -ApprovedRequest $approvedRequest -ScopeName "Loop2 UVM requirement changes" -RequiredSections @("requirements", "uvm", "test_plan")
        if (-not [string]::IsNullOrWhiteSpace($postChangeReason)) {
            Write-HdlHookDecision -Decision block -Continue $false -Reason $postChangeReason
            exit 0
        }
