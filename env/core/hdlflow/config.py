...
from .layout import GLOBAL_RULE_ROOT, find_workspace_root, project_config_path
from .simple_yaml import load_yaml


GLOBAL_CONFIG_FILES = {
    "workspace": str(GLOBAL_RULE_ROOT / "workspace_config.yaml"),
    "gates": str(GLOBAL_RULE_ROOT / "gates" / "global_gate_rules.yaml"),
    "toolchains": str(GLOBAL_RULE_ROOT / "toolchains" / "toolchains.yaml"),
    "agents": str(GLOBAL_RULE_ROOT / "agents" / "requirements_frontend_roles.yaml"),
    "naming": str(GLOBAL_RULE_ROOT / "naming" / "path_rules.yaml"),
    "reports": str(GLOBAL_RULE_ROOT / "reports" / "report_policy.yaml"),
    "snapshots": str(GLOBAL_RULE_ROOT / "snapshots" / "snapshot_policy.yaml"),
}
    ...
    for node_name, node_cfg in nodes.items():
        ...
        for section_name in ("inputs", "outputs"):
            ...
            for key, rel_path in section.items():
                errors.extend(_validate_project_rel_value(f"{node_name}.{section_name}.{key}", rel_path, allow_glob=False))
...
def _validate_source_policy(node_name: str, node_cfg: dict[str, Any]) -> list[str]:
    ...
    if not policy:
        if node_name in {"work/loop1_rtl_tb", "work/loop2_uvm", "work/loop3_fpga_proto"}:
            return [f"{node_name}.source_policy is required for Loop1/Loop2/Loop3 closure"]
        return []
    ...
    for section_name, section in policy.items():
        ...
        enforce_all_extensions = section.get("enforce_all_extensions")
        if enforce_all_extensions is not None and not isinstance(enforce_all_extensions, bool):
            errors.append(f"{node_name}.source_policy.{section_name}.enforce_all_extensions must be a boolean")
    return errors
...
def _validate_skill_policy(workspace_root: Path, node_name: str, node_cfg: dict[str, Any]) -> list[str]:
    ...
    if not policy:
        if node_name in {"work/loop1_rtl_tb", "work/loop2_uvm", "work/loop3_fpga_proto"}:
            return [f"{node_name}.skill_policy is required for Loop1/Loop2/Loop3 closure"]
        return []
    if not isinstance(policy, dict):
    ...
    for skill_name, spec in required.items():
        if not isinstance(spec, dict):
            ...
            continue
        path_value = spec.get("path") or f"env/rule/skills/{skill_name}/SKILL.md"
