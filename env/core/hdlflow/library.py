...
def build_library(workspace: Path) -> Path:
    ...
    library_root = workspace / LIB_ROOT
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        _import_uvm_guides(conn, library_root)
    return db_path
...
def get_entry(workspace: Path, entry_id: str, *, expected_kind: str | None = None) -> tuple[LibraryEntry, str]:
    ...
    if entry.detail_path:
        path = workspace.resolve() / LIB_ROOT / entry.detail_path
        if path.is_file():
...
def search_uvm_doc_chunks(
    ...
    *,
    ...
    db_path = _ensure_db(workspace)
    clauses = ["uvm_guide_chunks_fts MATCH ?"]
    params: list[Any] = [_fts_query(query_text)]
    if doc_id:
        clauses.append("doc_id = ?")
        params.append(doc_id)
    if tool_version:
        ...
        params.append(tool_version)
    sql = f"""
        SELECT chunk_id, doc_id, tool_version, anchor, text
        FROM uvm_guide_chunks_fts
        WHERE {' AND '.join(clauses)}
        LIMIT ?
    """
    params.append(limit)
    with closing(sqlite3.connect(db_path)) as conn:
        rows = conn.execute(sql, params).fetchall()
    keys = ["chunk_id", "doc_id", "tool_version", "anchor", "text"]
    return [dict(zip(keys, row)) for row in rows]


def query_uvm_examples(
    workspace: Path,
    *,
    keyword: str | None = None,
    doc_id: str | None = None,
    ...
    params: list[Any] = []
    if keyword:
        clauses.append("(caption LIKE ? OR code LIKE ?)")
        params.extend([f"%{keyword}%"] * 2)
    if doc_id:
        clauses.append("doc_id = ?")
        params.append(doc_id)
    if language_hint:
        clauses.append("language_hint = ?")
        params.append(language_hint)
    where = f"WHERE {' AND '.join(clauses)}" if clauses else ""
    sql = f"""
        SELECT example_id, doc_id, page, language_hint, caption, code
        FROM uvm_guide_examples
        {where}
        ORDER BY page, example_id
        LIMIT ?
    """
    params.append(limit)
    with closing(sqlite3.connect(db_path)) as conn:
        rows = conn.execute(sql, params).fetchall()
    keys = ["example_id", "doc_id", "page", "language_hint", "caption", "code"]
    return [dict(zip(keys, row)) for row in rows]
...
def format_uvm_example_rows(rows: list[dict[str, Any]]) -> list[str]:
    if not rows:
        return ["no matching UVM examples"]
    lines = ["example_id | doc | page | language | caption | preview"]
    lines.append("--- | --- | --- | --- | --- | ---")
    for row in rows:
        lines.append(
            " | ".join(
                [
                    ...
                    str(row.get("doc_id") or ""),
                    str(row.get("page") or ""),
                    str(row.get("language_hint") or ""),
                    _one_line(row.get("caption") or ""),
                    _one_line(row.get("code") or ""),
                ]
            )
        )
...
def _import_uvm_guides(conn: sqlite3.Connection, library_root: Path) -> None:
    parsed_root = library_root / "parsed" / "uvm_guides"
    if not parsed_root.is_dir():
        return
    for metadata_path in parsed_root.glob("*/*/metadata.json"):
        parsed_dir = metadata_path.parent
        ...
        conn.execute(
            """
            INSERT OR REPLACE INTO uvm_guide_documents (
                doc_id, title, vendor, tool_version, source_file, source_type,
                parser, page_count, section_count, chunk_count, example_count,
                parsed_dir, notes
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
        ...
        if sections_path.is_file():
            sections_data = json.loads(sections_path.read_text(encoding="utf-8"))
            for index, section in enumerate(sections_data.get("sections", []), start=1):
                title = str(section.get("title") or "")
                page = section.get("page")
                anchor = str(section.get("anchor") or "")
                section_id = f"{doc_id}.section_{index:04d}"
                conn.execute(
                    """
                    INSERT OR REPLACE INTO uvm_guide_sections (
                        section_id, doc_id, title, page, level, anchor
                    ) VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    (section_id, doc_id, title, page, str(section.get("level") or ""), anchor),
                )
        if examples_path.is_file():
            examples_data = json.loads(examples_path.read_text(encoding="utf-8"))
            for example in examples_data.get("examples", []):
                ...
                conn.execute(
                    """
                    INSERT OR REPLACE INTO uvm_guide_examples (
                        example_id, doc_id, page, language_hint, caption, code
                    ) VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    (
                        example_id,
                        str(example.get("doc_id") or doc_id),
                        example.get("page"),
                        str(example.get("language_hint") or ""),
            ...
            with chunks_path.open("r", encoding="utf-8") as handle:
                for line in handle:
                    ...
                    values = (
                        chunk_id,
                        str(chunk.get("doc_id") or doc_id),
                        str(chunk.get("domain") or "uvm"),
                        str(chunk.get("tool") or "uvm"),
                        str(chunk.get("tool_version") or ""),
                        str(chunk.get("section_type") or ""),
                        str(chunk.get("anchor") or ""),
                        chunk.get("page_start"),
                    ...
                    conn.execute(
                        """
                        INSERT OR REPLACE INTO uvm_guide_chunks (
                            chunk_id, doc_id, domain, tool, tool_version,
                            section_type, anchor, page_start, page_end, text,
                            source_file
                        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        """,
                        values,
                    ...
                    conn.execute(
                        """
                        INSERT INTO uvm_guide_chunks_fts (
                            chunk_id, doc_id, tool_version, anchor, text
                        ) VALUES (?, ?, ?, ?, ?)
                        """,
                        (values[0], values[1], values[4], values[6], values[9]),
                    )
...
def _ensure_db(workspace: Path) -> Path:
    db_path = workspace.resolve() / LIB_ROOT / "local" / "library.sqlite"
