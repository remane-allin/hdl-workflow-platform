...
FORMAL_REQUIREMENT_CHANGE_RULES = (
    {
        ...
        "required_sections": ("requirements", "rtl", "test_plan"),
    },
    {
        "node": "work/loop2_uvm",
        ...
        "required_sections": ("requirements", "uvm", "test_plan"),
    },
)
...
def evaluate_command_frontdoor_guard(project_path: Path, command: str) -> FrontdoorGuardResult:
    ...
    if _looks_like_write(command) and _writes_design_report_directly(command):
        return FrontdoorGuardResult(
            False,
            "design reports must be generated by python -m hdlflow.cli generate-design-doc after requirements-frontdoor-check passes",
        )
...
def require_change_request_before_frontdoor_source_write(project_path: Path) -> FrontdoorGuardResult:
    ...
    return FrontdoorGuardResult(
        ...
        (
            ...
            "rerunning requirements-frontdoor-check, generate-design-doc, and the DocParse gate"
        ),
    )
...
def require_change_request_before_prototype_change(project_path: Path) -> FrontdoorGuardResult:
    ...
    frontdoor_ready, frontdoor_reason = _has_post_change_frontdoor_and_design_doc(
        ...
        required_sections=("requirements", "fpga"),
    )
    if not frontdoor_ready:
        return FrontdoorGuardResult(False, frontdoor_reason)
    return FrontdoorGuardResult(
        True,
        "prototype change is bound to an approved front-door change with regenerated requirements/design documents",
    )
...
def _change_records_describe_delta(project: Path, change_id: str) -> bool:
    ...
    required_sections = ("## Requirements", "## Artifacts", "## Required Verification", "## Design Document Decision")
...
def _has_post_change_frontdoor_and_design_doc(
    ...
    *,
    ...
    design_report = project / "output" / "reports" / "design" / "design_rule_and_architecture.md"
    design_manifest = project / "output" / "reports" / "design" / "design_doc_manifest.json"
    if not design_report.exists() or not design_manifest.exists():
        return (
            False,
            (
                f"{scope} require regenerating the requirements/design document with "
                "python -m hdlflow.cli generate-design-doc before formal source edits"
            ),
        )
    if design_report.stat().st_mtime < request_mtime or design_manifest.stat().st_mtime < request_mtime:
        return (
            False,
            (
                f"{scope} require generate-design-doc to be rerun "
                "after the approved change before formal source edits"
            ),
        )
    try:
        ...
        return (
            False,
            f"{scope} require a valid design_doc_manifest.json before formal source edits",
        )
    sections = set(manifest.get("sections", []))
    missing_sections = sorted(set(required_sections) - sections)
    if missing_sections:
        return (
            False,
            f"{scope} require generated document section(s): {', '.join(missing_sections)}",
        )
    return True, "front-door and generated design document are current for the approved change"
...
def _controlled_command_stage_gate(project_path: Path, command: str) -> FrontdoorGuardResult:
    ...
    if _modelsim_loop(normalized) == "loop1" or "loop1-refresh-reports" in normalized or "loop1-waveform-check" in normalized:
...
def _writes_design_report_directly(command: str) -> bool:
    ...
    if DESIGN_REPORT_PREFIX not in normalized:
        return False
    return not re.search(r"\bgenerate-design-doc\b", normalized)
