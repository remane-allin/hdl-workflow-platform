...
def refresh_loop2_reports(project_path: Path) -> Loop2ReportResult:
    ...
    runtime_dir = project / "work/loop2_uvm" / "_runtime"
    ...
    _require_real_uvm_log(log_text, log_path)
    ...
    _require_real_coverage_report(coverage_text, report_coverage)
...
def _extract_metrics(log_text: str, coverage_text: str) -> dict[str, object]:
    ...
    test_cases = _extract_test_cases(log_text)
    return {
...
def _is_pass(metrics: dict[str, object]) -> bool:
    ...
    structured_cases_pass = True
    if isinstance(test_cases, list) and test_cases:
        structured_cases_pass = all(str(case.get("result")) == "PASS" for case in test_cases if isinstance(case, dict))
    return (
        ...
        and structured_cases_pass
    )


def _require_real_uvm_log(log_text: str, log_path: Path) -> None:
    required_markers = [
    ...
    if "HDLFLOW_TEST_CASE" not in log_text:
        missing.append("HDLFLOW_TEST_CASE")
    if missing:
        raise ValueError(f"{log_path} is not a complete Loop2 UVM transcript; missing marker(s): {', '.join(missing)}")
    if len(log_text) < 5000:
        raise ValueError(f"{log_path} is too small to be accepted as Loop2 UVM run evidence")


def _require_real_coverage_report(coverage_text: str, coverage_path: Path) -> None:
    required_markers = [
        "TOTAL COVERGROUP COVERAGE",
        "Total Coverage By File (code coverage only, filtered view)",
        "=== File:",
    ]
    missing = [marker for marker in required_markers if marker not in coverage_text]
    if missing:
        raise ValueError(f"{coverage_path} is not a complete ModelSim coverage report; missing marker(s): {', '.join(missing)}")
    if len(coverage_text) < 1000:
        raise ValueError(f"{coverage_path} is too small to be accepted as Loop2 coverage evidence")


def _extract_test_cases(log_text: str) -> list[dict[str, str]]:
    cases: list[dict[str, str]] = []
    for line in log_text.splitlines():
        if "HDLFLOW_TEST_CASE" not in line:
            continue
        fields = _parse_fields(line)
        test_id = fields.get("id") or fields.get("name")
...
def _format_regression_report(project: str, generated_at: str, result: str, metrics: dict[str, object]) -> str:
    ...
    cases = metrics.get("test_cases")
    if isinstance(cases, list) and cases:
        for case in cases:
            if not isinstance(case, dict):
                continue
            lines.append(
                f"| {case.get('item', '-')} | {case.get('stimulus', '-')} | {case.get('expected', '-')} | {case.get('actual', '-')} | {case.get('result', '-')} |"
            )
    else:
        lines.extend(
            [
    ...
    structured = cases if isinstance(cases, list) and cases else [
        {
            "item": "ordered_loopback_stream",
            "stimulus": f"{metrics['transactions_total']} sequencer-generated transactions",
            "expected": "same ordered transaction stream at monitor/scoreboard",
            "actual": f"{metrics['scoreboard_matches']} scoreboard matches",
            "result": "PASS" if metrics["scoreboard_pass"] else "FAIL",
            "transactions": str(metrics["transactions_total"]),
            "stimuli": "-",
        }
    ]
    for index, case in enumerate(structured, start=1):
        if not isinstance(case, dict):
            continue
        lines.extend(
            [
                f"========== LOOP2_TEST_ITEM_{index:03d} START ==========",
                f"- test_item: {case.get('item', '-')}",
                f"- stimulus_sent: {case.get('stimulus', '-')}",
                f"- expected_result: {case.get('expected', '-')}",
                f"- actual_result: {case.get('actual', '-')}",
                f"- transactions: {case.get('transactions', '-')}",
                f"- stimuli: {case.get('stimuli', '-')}",
                f"- result: {case.get('result', '-')}",
                f"========== LOOP2_TEST_ITEM_{index:03d} END ==========",
                "",
            ]
        )
    lines.extend(
        [
            ...
            "- template_artifacts: none under output/uvm",
