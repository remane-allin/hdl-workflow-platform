...
4. Treat the simulator log as the report source. Loop1 and Loop2 final reports must be regenerated by the local report script from `HDLFLOW_TEST_CASE` markers, not hand-edited.
   For Loop1, `rtl_functional.do` must also record WLF/VCD evidence under `output/sim/loop1/wave`, emit `HDLFLOW_WAVE_GROUP` scope markers, and run `loop1-waveform-check` after `loop1-refresh-reports`.
