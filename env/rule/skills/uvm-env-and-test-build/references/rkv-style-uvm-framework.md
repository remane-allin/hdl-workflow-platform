...
Keep root `output/uvm/` free of implementation files except an optional short
...
6. Search for old root paths such as:
   - `output/uvm/<dut>_uvm_pkg.sv`
   - `output/uvm/<dut>_<protocol>_if.sv`
   - `output/uvm/tb_<dut>_uvm.sv`
...
- The latest evidence path in review or `work/memory/` files points to the newest passing
