...
4. If the requirements front door is missing or stale, run `requirements-frontdoor-init`, normalize the five machine-readable structured spec files (`interface_spec.yaml`, `interface_timing.yaml`, `register_map.yaml`, `test_intent.yaml`, and `timing_rules.yaml`), and route to `$hdl-requirements-decompose`. After the active baseline changes, run `python -m hdlflow.cli semantic-compile --project <project>` before `generate-docs`; this compiles `active_requirements`, `functional_domain_model`, `design_routing`, `module_ownership_matrix`, `operation_model`, `req_to_rtl_implementation`, `interface_contract`, `tb_obligations`, `wave_semantic_manifest`, `uvm_obligations`, and `fpga_validation_matrix`. These generated artifacts are obligations only; `planned` rows must remain release-blocking until real evidence exists. During this stage, Spec Agent must record unresolved doubts directly in `work/docparse/structured_spec/document_analysis.yaml.open_questions`, send them to the user for review, and record `question_review.status: REVIEWED`, `review_evidence: work/docparse/structured_spec/document_analysis.yaml`, and `unresolved_count: 0` before READY. Sim Agent must plan Loop1 waveform observability in `test_intent.yaml.waveform_windows` and `verification_plan.yaml.waveform_comparison`; `generate-docs` must carry this plan into `output/docs/test/verification_plan.md` before Loop1 work begins.
...
6. Before entering Loop1, read `$rtl-architecture-and-gen` and its style guide once for constraints. Keep the directed full-function TB plan in `work/docparse/verification/verification_plan.yaml` and the generated user-facing plan in `output/docs/test/verification_plan.md`; do not create a sidecar TB plan Markdown under `output/tb/`. RTL/TB/UVM/FPGA work must trace through `work/docparse/verification/operation_model.yaml`, `work/loop1_rtl_tb/trace_matrix/req_to_rtl_implementation.yaml`, `work/loop1_rtl_tb/config/interface_contract.yaml`, `work/loop1_rtl_tb/config/tb_obligations.yaml`, `work/loop1_rtl_tb/config/wave_semantic_manifest.yaml`, `work/loop2_uvm/config/uvm_obligations.yaml`, and `work/loop3_fpga_proto/config/fpga_validation_matrix.yaml`. Directed TBs must emit `HDLFLOW|CHECK|schema=hdlflow_event_v1|version=1|stage=loop1|...` and `HDLFLOW|SUMMARY|schema=hdlflow_event_v1|version=1|stage=loop1|...` events plus `requirement_id`, `operation_id`, `observed_interface`, `evidence_type`, readback, and side-effect fields for requirement evidence. Waveform window markers (`HDLFLOW_WAVE_BEGIN`/`HDLFLOW_WAVE_END` or `HDLFLOW_WAVE_WINDOW`) let `loop1-waveform-gate` produce waveform quality evidence, while `loop1-waveform-semantic` decodes `work/loop1_rtl_tb/config/wave_semantic_manifest.yaml` into `output/reports/loop1/waveform_semantic_report.json`. Loop1 WLF/VCD capture is a deliverable under `output/sim/loop1/wave/`; analysis must use those files in place and must not make `_runtime` waveform copies. Loop1 hard pass is `loop1_deterministic_gate`: TB report PASS, parser clean, failed checks/tests zero, simulator errors zero, fatal markers zero, and assertion PASS when assertions are enabled. Waveform activity/quality evidence alone is not requirement evidence; only verification-level semantic windows with decoded expected events can count toward waveform semantic closure. After each `.v` file is generated or edited, run `python -m hdlflow.cli rtl-skill-audit --project <project>`; the platform-generated `output/reports/loop1/rtl_skill_audit.md` is the only valid RTL skill audit evidence. Review Agent must then add a structured finding for every formal implementation surface it reviews: RTL cites the audit and RTL skill/style guide, directed TB cites `output/docs/test/verification_plan.md` and Loop1/ModelSim skill, UVM cites `uvm-env-and-test-build`, and Loop3 cites the prototype preflight/report refresh commands.
