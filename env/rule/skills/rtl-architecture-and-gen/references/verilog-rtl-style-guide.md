...
Source:
- Project Verilog coding-standard references under `input/spec/` or the local project document area
...
- RTL `.v` files under `output/rtl/` must not declare Verilog `task` or `endtask`.
- A synthesizable `function` must remain purely combinational and use blocking assignments only.
- Directed TB `.v` files under `output/tb/` may use Verilog `task` helpers for stimulus, waits, score checks, and log formatting.
...
- Keep RTL function definitions local unless they are reviewed shared utilities. Keep TB task helpers inside TB-owned files.
...
- Keep field/register updates, FIFO access, and datapath mutation in owned enable-controlled logic instead of embedding every side effect in the FSM body.
...
- The file header description must match the module's real ownership. Register files, CSR blocks, control/status modules, FIFO status mappers, datapaths, and protocol controllers must name those responsibilities explicitly.
- Do not leave stale or overly narrow descriptions after refactoring. For example, a module that owns control/status register behavior must not be described only as flag mapping.
- Do not add large filler note blocks for comment-count compliance. Use concise ownership, protocol, timing, reset, CDC, register, and exception comments instead.
...
- No file-level FSM owns all command, field, FIFO, register, datapath, and control behavior.
...
- Synthesizable reusable RTL logic uses `function`; RTL `.v` files under `output/rtl/` contain no `task` or `endtask` declarations.
...
- Header description and scope match the actual register/control/status/datapath/protocol ownership found in the file.
