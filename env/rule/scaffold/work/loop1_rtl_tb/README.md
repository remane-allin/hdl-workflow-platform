...
`loop1_waveform_query_gate` validates those windows before Loop2 may start. The
semantic waveform query gate parses `config/top_wave_manifest.yaml`, checks that
the VCD is limited to DUT top-level observability, and writes
