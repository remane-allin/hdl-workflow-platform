...
reported as a blocking gate failure and overrides a deterministic TB PASS until
the required task-bound top-port evidence is present.
Review `waveform_query_report.md`, `waveform_gate.json`, and
`query_transcript.json` when the blocking gate fails.
