...
- generated_at: 2026-06-29T22:41:12
- project: Hi3593_v2
- node: work/docparse
- level: develop
...
| change_control_state | FAIL | approved change request records are incomplete: RTL changes must include required verification item(s): review-check, run-gate --node loop1; Loop1 TB changes must include required verification item(s): review-check, run-gate --node loop1; Loop2 UVM changes must include required verification item(s): review-check, run-gate --node loop2; Loop3 prototype changes must include required verification item(s): prototype-preflight, validate-prototype-plan, loop3-refresh-reports, review-check, run-gate --node loop3 |
| frontdoor_execution_lock | PASS | frontdoor intake may be edited only before DocParse exit |
| automatic_report_refresh | PASS | skipped because an upstream gate precondition failed; evidence was not regenerated |
